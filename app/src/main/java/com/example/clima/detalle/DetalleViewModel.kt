...
class DetalleViewModel(private val nombre: String, val db: CiudadDatabaseDao): ViewModel(){
    ...
    val aviso: LiveData<Int>
        get()=_aviso
    //maneja la visibilidad de los datos, si NO existe, no muestra nada
    private val _datos= MutableLiveData<Int>()
    val datos: LiveData<Int>
        get()=_datos
    ...
    private var tiempo= MutableLiveData<Weather>()
...
//                tiempo= CiudadApi
//                    .retrofitService
//                    .getWeather(nombre, "710b9a74fd5943aff012c7e3e83be945") as MutableLiveData<Weather>
//                _notificacion.value=""
//                //conversiones
...
//                _notificacion.value="Sin conexion a Internet, se muestra el ultimo registro guardado."
//                _aviso.value= View.VISIBLE
//                onBuscarDatosCiudad()
//                _weather.value= tiempoDB
//                if(e.message=="HTTP 404 Not Found"){//como la ciudad no existe se procede a eliminar
//                    _datos.value= View.GONE
